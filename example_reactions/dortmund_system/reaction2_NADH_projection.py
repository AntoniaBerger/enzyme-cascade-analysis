...
from noise_function_libary import add_noise_plate_reader, add_noise_rate, full_experiment_processing_with_noise
...
def michaelis_menten_inhibition_NADH(S, *parameters):
    S1 = S
    Vmax, Km1 = parameters
    return (Vmax * S1 * 300) / ((3 + 300) * (Km1 + S1))
...
df_reaction2_noisy_plate_reader = save_results(
    ...
    parameters, 
    save_path=os.path.join(RESULTS_PATH, "experimental_reaction2_NADH_noisy_plate_reader_results.csv")
)
...
df_reaction2_noisy_rate = save_results(
    ...
    parameters, 
    save_path=os.path.join(RESULTS_PATH, "experimental_reaction2_NADH_noisy_rate_results.csv")
)
